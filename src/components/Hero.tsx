  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        style={{ y: bgY }}
      >
        <div className="absolute top-[10%] left-[5%] w-[300px] h-[300px] md:w-[600px] md:h-[600px] rounded-full bg-violet-accent/5 blur-[80px] md:blur-[120px]" />
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 w-full grid grid-cols-1 lg:grid-cols-12 gap-12 items-center flex-grow">
        ...
        >
          {/* Subtitle Line 1 */}
          <span className="uppercase tracking-[0.25em] text-[11px] md:text-[13px] font-semibold text-violet-accent mb-4 block">
            Blockchain & ZK Engineer
          </span>

          {/* Headline Line 2 */}
          <h1 className="font-space font-bold text-5xl sm:text-6xl md:text-8xl tracking-tight text-white leading-none mb-6">
            Abhii Navv
          </h1>

          {/* Description Line 3 */}
          <p className="text-white/60 text-base sm:text-lg md:text-xl font-normal leading-relaxed max-w-xl mb-10">
          ...
          <div className="flex flex-row items-center gap-4 sm:gap-6 w-full sm:w-auto">
            <button
              ...
              className="px-6 py-3 md:px-8 md:py-4 bg-violet-accent text-white font-medium rounded-lg hover:bg-violet-accent/90 transition-all duration-300 shadow-lg shadow-violet-accent/20 text-sm md:text-base cursor-pointer"
            >
              View Projects
            </button>
            <a
            ...
            >
              GitHub <ArrowUpRight className="w-4 h-4 text-violet-accent" />
            </a>
          </div>
        </motion.div>
          ...
          >
            ...
            <defs>
              <pattern id="dotPattern" width="10" height="10" patternUnits="userSpaceOnUse">
                <circle cx="2" cy="2" r="1" fill="#7C3AED" fillOpacity="0.3" />
              </pattern>
            </defs>

            {/* Glowing Center */}
            <circle cx="100" cy="100" r="12" fill="#7C3AED" className="blur-[4px]" fillOpacity="0.3" />
            <circle cx="100" cy="100" r="4" fill="#ffffff" fillOpacity="0.8" />

            {/* Inner Ring Hexagons */}
            <polygon points="100,60 135,80 135,120 100,140 65,120 65,80" fill="none" stroke="#7C3AED" strokeWidth="0.75" strokeDasharray="3 3" />
            <polygon points="100,40 152,70 152,130 100,160 48,130 48,70" fill="none" stroke="#ffffff" strokeWidth="0.5" strokeOpacity="0.5" />
            <polygon points="100,20 169,60 169,140 100,180 31,140 31,60" fill="none" stroke="#7C3AED" strokeWidth="0.75" strokeOpacity="0.8" />

            {/* Radial Lines connecting vertices */}
            <line x1="100" y1="20" x2="100" y2="180" stroke="#7C3AED" strokeWidth="0.25" strokeOpacity="0.4" />
            <line x1="31" y1="60" x2="169" y2="140" stroke="#7C3AED" strokeWidth="0.25" strokeOpacity="0.4" />
            <line x1="31" y1="140" x2="169" y2="60" stroke="#7C3AED" strokeWidth="0.25" strokeOpacity="0.4" />

            {/* Decorative Orbiting Node Rings */}
            <circle cx="100" cy="20" r="3" fill="#ffffff" />
            <circle cx="169" cy="60" r="2" fill="#7C3AED" />
            <circle cx="169" cy="140" r="3" fill="#ffffff" />
            <circle cx="100" cy="180" r="2" fill="#7C3AED" />
            <circle cx="31" cy="140" r="3" fill="#ffffff" />
            <circle cx="31" cy="60" r="2" fill="#7C3AED" />
      ...
      <div className="w-full bg-[#0d0d14]/60 border-y border-white/5 py-4 overflow-hidden relative z-10">
        <div className="flex whitespace-nowrap">
          {/* Double list for smooth seamless looping */}
          <div className="animate-ticker flex items-center">
            {tickerItems.concat(tickerItems).map((item, idx) => (
              <div key={idx} className="flex items-center text-sm md:text-base font-space uppercase tracking-widest text-white/50 mx-8">
                <span>{item}</span>
                <span className="w-2 h-2 rounded-full bg-violet-accent ml-16" />
              </div>
            ))}
          </div>
        </div>
